...
4. 1 行目にヘッダーを入力（例）:
   - **予約シート（1枚目）:** `ID` | `名前` | `メール` | `電話` | `メニュー` | `日付` | `時間` | `登録日時`
5. **お問い合わせも同じスプレッドシートで受けたい場合:** 同じブックにシートを追加し、名前を **「お問い合わせ」** にする。1 行目に `登録日時` | `名前` | `メール` | `件名` | `メッセージ` を入力
6. ダウンロードした JSON 鍵の中の **`client_email`** をコピーし、そのメールアドレスを **スプレッドシートの共有** に追加（**編集者** 権限）
...
設定後、**予約**は「予約」シートに、**お問い合わせ**は「お問い合わせ」シートに、それぞれ次の行へ自動で追記されます。
